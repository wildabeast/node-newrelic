...
function storeTxInfo(transaction, request, response) {
  if (!request || !response) {
    console.log('Missing request or response object! Not storing transaction info.')
    return
  }
...
function wrapResponseEnd(agent, proto) {
  ...
  shimmer.wrapMethod(proto, 'Response.prototype', 'end', function wrapResEnd(end) {
    if (typeof end !== 'function') {
      console.log('Response#end is not a function?')
      return end
    }
...
module.exports = function initialize(agent, http, moduleName) {
  if (!http) {
    console.log('Did not get http module, not instrumenting!')
    return false
  }
