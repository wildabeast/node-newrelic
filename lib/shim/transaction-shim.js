...
function bindCreateTransaction(nodule, property, spec) {
  ...
  return this.wrap(nodule, property, function makeTransWrapper(shim, fn, name) {
    if (!shim.isFunction(fn)) {
      shim.console.log('Not wrapping "%s" with transaction, not a function.', name)
      return fn
    }
...
function handleMqTracingHeaders(headers, segment, transportType) {
  ...
  if (!headers) {
    this.console.log('No headers for CAT or DT processing.')
    return
  }
...
function insertCATRequestHeaders(headers, useAlternateHeaderNames) {
  ...
  if (!headers) {
    this.console.log('Missing headers object, not adding headers!')
    return
  }
...
function insertCATReplyHeader(headers, useAlternateHeaderNames) {
  ...
  if (!config.cross_application_tracer.enabled) {
    ...
    this.console.log('Distributed tracing is enabled, not adding CAT reply header.')
    return
  } else if (!config.encoding_key) {
    this.console.log('Missing encoding key, not adding CAT reply header!')
    return
  } else if (!headers) {
    this.console.log('Missing headers object, not adding CAT reply header!')
    return
  }
