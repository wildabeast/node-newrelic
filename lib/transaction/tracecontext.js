...
class TraceContext {
  ...
  createTracestate() {
    ...
    if (config.span_events.enabled) {
      ...
      if (segment) {
        ...
        console.log('No segment/span in context. Not sending spanId in tracestate.')
      }
    } else {
  ...
  acceptTraceContextPayload(traceparent, tracestate) {
    ...
    if (parsedState.entryValid) {
      ...
      console.error('Invalid tracestate for transaction %s: %s', this.transaction.id, tracestate)
    ...
    for (let i = 0; i < incomingListMembers.length; i++) {
      ...
      if (listMember !== '') {
        ...
        if (listMemberParts.length !== 2) {
          console.log('Unable to parse tracestate list members.')
