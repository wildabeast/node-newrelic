...
module.exports.getBootId = function getBootId(agent, callback) {
  if (!/linux/i.test(os.platform())) {
    console.log('Platform is not a flavor of linux, omitting boot info')
...
function fetchDockerVendorInfo(agent, callback) {
  ...
  if (!os.platform().match(/linux/i)) {
    console.log('Platform is not a flavor of linux, omitting docker info')
    return callback(null, null)
  }
...
function parseCGroupsV2(data, callback) {
  ...
  if (line) {
    ...
    console.log(`Found ${CGROUPS_V2_PATH} but failed to parse Docker container id.`)
    callback(null, null)
  }
}
...
function findCGroupsV1(callback) {
  common.readProc(CGROUPS_V1_PATH, function getCGroup(err, data) {
    if (!data) {
      console.log(`${CGROUPS_V1_PATH} not found, exiting parsing containerId.`)
    ...
    if (id) {
      ...
      console.log('No matching cpu group found.')
      callback(null, null)
    }
  })
}
