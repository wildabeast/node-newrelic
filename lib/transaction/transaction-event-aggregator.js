...
class TransactionEventAggregator extends EventAggregator {
  ...
  _toPayloadSync() {
    ...
    if (events.length === 0) {
      console.log('No transaction events to send.')
      return
    }
  ...
  send() {
    ...
    console.log('Splitting transaction events into multiple payloads')
  ...
  async _sendMultiple(eventPayloadPairs, sendCallback) {
    ...
    const promises = eventPayloadPairs.map((payloadPair, index) => {
      ...
      return new Promise((resolve) => {
        self._sendSplitPayload(payloadPair.rawData, payloadPair.payload, (error) => {
          if (error) {
            console.log(error, 'An error occurred sending payload')
          }
    ...
    console.log('Finished sending %d payloads to %s', eventPayloadPairs.length, self.method)
