...
class ErrorTraceAggregator extends TraceAggregator {
  ...
  add(error) {
    if (this.errors.length < this.limit) {
      console.log(error, 'Error to be sent to collector.')
      this.errors.push(error)
    } else {
      console.log('Already have %d errors to send to collector, not keeping.', this.limit)
    }
  }
  ...
  _toPayloadSync() {
    ...
    console.log('No error traces to send.')
  }
  ...
  _merge(errors) {
    ...
    console.log('Merging %s (of %s) errors for next delivery.', len, errors.length)
