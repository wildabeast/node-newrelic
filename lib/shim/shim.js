...
function wrap(nodule, properties, spec, args) {
  if (!nodule) {
    this.console.log('Not wrapping non-existent nodule.')
    return nodule
  }
  ...
  properties.forEach(function wrapEachProperty(prop) {
    ...
    if (!original) {
      this.console.log('Not wrapping missing property "%s"', prop)
      return
    }
...
function record(nodule, properties, recordNamer) {
  ...
  return this.wrap(nodule, properties, function makeWrapper(shim, fn, name) {
    ...
    if (!shim.isFunction(fn)) {
      shim.console.log('Not recording non-function "%s".', name)
      return fn
    }
...
function recordWrapper({ shim, fn, name, recordNamer }) {
  return function wrapper() {
    ...
    if (!parent) {
      shim.console.log('Not recording function %s, not in a transaction.', name)
...
function bindSegment(nodule, property, segment, full) {
  ...
  if (segment != null && !this.isObject(segment)) {
    this.console.log({ segment: segment }, 'Segment is not a segment, not binding.')
    return nodule
  }
...
function _rawCreateSegment(shim, opts) {
  ...
  if (segment) {
    ...
    shim.console.log(opts, 'Failed to create segment')
  }
...
function enableDebug() {
  this.console.log('Enabling debug mode for shim!')
  this._debug = true
  this._wrapped = []
}
...
function unwrapAll() {
  if (this._wrapped) {
    this.console.log('Unwrapping %d items.', this._wrapped.length)
