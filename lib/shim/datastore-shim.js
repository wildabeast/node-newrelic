...
function bindRowCallbackSegment(args, cbIdx, parentSegment) {
  ...
  if (idx === null) {
    this.console.log('Not binding row callback, invalid cbIdx %s', cbIdx)
    return
  }
  ...
  if (!this.isFunction(cb)) {
    this.console.log('Argument %d is not a function, not binding row callback', cbIdx)
    return cb
  }
...
function _recordQuery(suffix, nodule, properties, querySpec) {
  ...
  if (!querySpec) {
    this.console.log('Missing query spec for recordQuery, not wrapping.')
    return nodule
  }
...
function _extractQueryStr(fn, fnName, spec, ctx, args) {
  ...
  if (this.isNumber(queryStr)) {
    ...
    if (queryIdx === null) {
      this.console.log('Invalid query index %d of %d', queryStr, args.length)
      return null
    }
