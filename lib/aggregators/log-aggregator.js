...
class LogAggregator extends EventAggregator {
  ...
  _toPayloadSync() {
    ...
    if (events.length === 0) {
      console.log('No log events to send.')
      return
    }
    ...
    if (!formattedLogs.length) {
      console.log('No log events to send after formatting.')
      return
    }
  ...
  reconfigure(config) {
    ...
    if (this.limit <= 0 && oldLimit > 0) {
      console.log('This New Relic account has disabled APM logs.')
      this.clear()
    } else if (oldLimit <= 0 && this.limit > 0) {
      console.log('This New Relic account has re-enabled APM logs.')
    }
  }
}
