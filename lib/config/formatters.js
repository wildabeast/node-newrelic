...
formatters.object = function object(val, logger) {
  try {
    ...
    console.error('New Relic configurator could not deserialize object:')
...
formatters.objectList = function objectList(val, logger) {
  try {
    ...
    console.error('New Relic configurator could not deserialize object list:')
    console.error(error.stack)
  }
}
...
formatters.regex = function regex(val, logger) {
  try {
    ...
    console.error(`New Relic configurator could not validate regex: ${val}`)
    console.error(error.stack)
  }
}
