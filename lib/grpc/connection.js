...
class GrpcConnection extends EventEmitter {
  ...
  connectSpans() {
    ...
    try {
      ...
      console.log(err, 'Unexpected error establishing gRPC stream, will not attempt reconnect.')
      this._disconnect()
    }
  }
  ...
  _setupSpanStreamObservers(stream) {
    ...
    stream.on('error', (err) => {
      ...
      console.log('Span stream error. Code: [%s]: %s', err.code, err.details)
    })
  }
  ...
  _generateCredentials(grpcApi) {
    ...
    if (rootCerts) {
      console.log('Infinite tracing root certificates found to attach to requests.')
      try {
        certBuffer = Buffer.from(rootCerts, 'utf-8')
      } catch (err) {
        console.log('Failed to create buffer from rootCerts, proceeding without.', err)
      }
    }
