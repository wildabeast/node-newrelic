...
async function listPackages(root, packages = []) {
  ...
  async function forEachDir(dir) {
    ...
    try {
      ...
      console.log(err, 'Could not read %s.', pkg)
    }
...
async function listDependencies(root, children = [], visited = Object.create(null)) {
  ...
  async function forEachEntry(entry) {
    ...
    try {
      ...
      if (err.code !== 'ENOENT') {
        console.log(err, 'Failed to resolve candidate real path %s', candidate)
      }
  ...
  return Object.keys(info)
    ...
    .map((pair) => {
      try {
        ...
        console.log(err, 'Unable to stringify package version')
        return '<unknown>'
      }
    })
}
