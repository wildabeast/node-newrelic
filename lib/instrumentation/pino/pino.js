...
module.exports = function instrument(shim, tools) {
  ...
  if (semver.lt(pinoVersion, '7.0.0')) {
    shim.console.log('Instrumentation only supported on pino >=7.0.0.')
    return
  }
  ...
  if (!isApplicationLoggingEnabled(config)) {
    shim.console.log('Application logging not enabled. Not instrumenting pino.')
    return
  }
...
function reformatLogLine({ logLine, msg, agent, chindings = '', level, logger }) {
  ...
  return function formatLogLine() {
    ...
    try {
      ...
      console.error('Failed to parse log line as json: %s', err.message)
      return
    }
