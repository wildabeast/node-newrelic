...
exports.getKeys = function getKeys(data, keys) {
  ...
  for (let i = 0; i < keys.length; ++i) {
    ...
    if (!properties.hasOwn(data, key) || !data[key]) {
      console.log('Key %s missing from metadata', key)
      return null
    }
    let value = data[key]
    ...
    if (!checkValueString(value)) {
      console.log('Invalid metadata value found: %s -> %s', key, value)
      return null
    }
    results[key] = value
  }
...
exports.request = function request(opts, agent, cb) {
  ...
  const req = http.get(opts, function awsRequest(res) {
    ...
    function respond(data) {
      ...
      if (res.statusCode !== 200) {
        ...
        console.log('Got no response data?')
  ...
  req.on('error', function requestError(err) {
    if (err.code === 'ECONNRESET') {
      console.log('Request for metadata %j timed out', opts)
      return cb(err)
    }

    console.log('Message for metadata %j: %s', opts, err.message)
    cb(err)
  })
  ...
  function abortRequest() {
    console.log('Aborting request for metadata at %j', opts)
    req.abort()
    agent.removeListener('errored', abortRequest)
    agent.removeListener('stopped', abortRequest)
    agent.removeListener('disconnected', abortRequest)
...
function readProc(path, callback) {
  fs.readFile(path, function readProcFile(err, data) {
    if (err) {
      console.error(err, 'Error when trying to read %s', path)
      callback(err, null)
    } else {
