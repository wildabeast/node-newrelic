...
function parseConnectionString(provider, connectionUrl) {
  ...
  try {
    ...
    console.log('Failed to parse connection string for %s: %s', provider, err.message)
  }
  return parameters
}
...
function extractQueryArgs(args, pkgVersion) {
  ...
  try {
    ...
    console.error('Failed to extract query from raw query: %s', err.message)
  }
        ...
        inContext: function inContext() {
          if (!client[prismaConnection]) {
            try {
              ...
              console.error('Failed to retrieve prisma config in %s: %s', pkgVersion, err.message)
