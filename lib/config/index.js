...
Config.prototype._alwaysUpdateIfChanged = function _alwaysUpdateIfChanged(json, key) {
  ...
  if (value != null && this[key] !== value) {
    ...
    this.emit(key, value)
    console.log('Configuration of %s was changed to %s by New Relic.', key, value)
  }
}
...
function _validateThenUpdateErrorMessages(remote, local, remoteKey, localKey) {
  const valueToTest = remote[remoteKey]
  if (Array.isArray(valueToTest)) {
    console.log('Saw SSC (ignore|expect)_message that is an Array, will not merge: %s', valueToTest)
    return
  }

  if (!valueToTest) {
    console.log('SSC ignore|expect_message is null or undefined, will not merge')
    return
  }
  ...
  Object.keys(valueToTest).forEach(function validateArray(key) {
    ...
    if (!Array.isArray(arrayToTest)) {
      console.log('Saw SSC message array that is not an array, will not merge: %s', arrayToTest)
      valid = false
    }
  })
  if (!valid) {
...
Config.prototype.logUnknown = function logUnknown(json, key) {
  const value = json[key]
  console.log('New Relic sent unknown configuration parameter %s with value %s.', key, value)
}
...
function getDisplayHost() {
  ...
  if (numBytes > 255) {
    console.log('Custom host display name must be less than 255 bytes')
    _displayHost = this.getHostnameSafe()
    return _displayHost
  }
...
Config.prototype._fromPassed = function _fromPassed(external, internal, arbitrary) {
  ...
  Object.keys(external).forEach(function overwrite(key) {
    ...
    try {
      ...
      console.log('Error thrown on access of user config for key: %s', key)
      return
    }
...
function _parseRange(range, parsed) {
  ...
  if (split.length !== 2) {
    console.log('Failed to parse range %s', range)
    return parsed
  }
  if (split[0] === '') {
  ...
  if (Number.isNaN(lower) || Number.isNaN(upper)) {
    console.log('Range must contain two numbers %s', range)
    return parsed
  }
  if (lower > upper) {
    console.log('Range must start with lower bound %s', range)
  } else if (lower < 0 || upper > 1000) {
    console.log('Range must be between 0 and 1000 %s', range)
  } else {
    // success
  ...
  for (let i = 0; i < codes.length; i++) {
    ...
    if (typeof code === 'string' && code.indexOf('-') !== -1) {
      ...
      if (!Number.isNaN(parsedCode)) {
        ...
        console.log('Failed to parse status code %s', code)
      }
    }
  }
  return parsedCodes
  ...
  function checkKey(base, target, settings, key) {
    ...
    if (hsValue && typeof hsValue === 'object' && !(hsValue instanceof Array)) {
      if (typeof target[key] !== 'object') {
        console.log('High Security Mode: %s should be an object, found %s', key, target[key])
    ...
    if (target[key] !== hsValue) {
      console.log('High Security Mode: %s was set to %s, coercing to %s', key, target[key], hsValue)
      target[key] = hsValue
...
function _laspReponse(keys) {
  if (keys.length) {
    console.error('The agent received one or more unexpected security policies and will shut down.')
...
Config.prototype.validateFlags = function validateFlags() {
  Object.keys(this.feature_flag).forEach(function forEachFlag(key) {
    if (featureFlag.released.indexOf(key) > -1) {
      console.log('Feature flag %s has been released', key)
    }
    if (featureFlag.unreleased.indexOf(key) > -1) {
      console.log('Feature flag %s has been deprecated', key)
    }
  })
}
...
Config.prototype.publicSettings = function publicSettings() {
  ...
  try {
    ...
    console.error(err, 'Unable to stringify settings object')
  }
}
...
function initialize(config) {
  ...
  console.log('Using configuration file %s.', filepath)
