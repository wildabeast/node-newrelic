...
Transaction.prototype.setForceIgnore = function setForceIgnore(ignore) {
  if (ignore != null) {
    ...
    console.log('Transaction#setForceIgnore called with null value')
  }
}
...
Transaction.prototype.finalizeName = function finalizeName(name) {
  ...
  if (!name) {
    console.log('No name for transaction %s, not finalizing.', this.id)
    return
  }
...
const _dtParseTest = function _dtParseTest(payload) {
  ...
  if (!parsed.v) {
    console.log('Received a distributed trace payload with no version field', this.id)
  }
  if (!parsed.d) {
    console.log('Received a distributed trace payload with no data field', this.id)
  }
...
const _dtVersionTest = function _dtVersionTest(parsed) {
  ...
  if (majorVersion === null) {
    console.log('Invalid distributed trace payload, not accepting')
    this.agent.recordSupportability(DT_ACCEPT_PAYLOAD_EXCEPTION_METRIC)
  }
  if (majorVersion > 0) {
...
function _acceptDistributedTracePayload(payload, transport) {
  ...
  if (!data) {
    console.log('No distributed trace data received, not accepting payload')
    this.agent.recordSupportability(DT_ACCEPT_PAYLOAD_EXCEPTION_METRIC)
    return
  }
  ...
  if (typeof payload === 'string') {
    if (payload.charAt(0) !== '{' && payload.charAt(0) !== '[') {
      try {
        ...
        console.log(err, 'Got unparseable distributed trace payload in transaction %s', this.id)
        this.agent.recordSupportability(DT_ACCEPT_PAYLOAD_PARSE_EXCEPTION_METRIC)
        return null
      }
    }
    try {
      ...
      console.log(err, 'Failed to parse distributed trace payload in transaction %s', this.id)
      this.agent.recordSupportability(DT_ACCEPT_PAYLOAD_PARSE_EXCEPTION_METRIC)
      return null
    }
  }
