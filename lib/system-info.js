...
module.exports._getProcessorStats = async function getProcessorStats() {
  ...
  console.log('Unknown platform: %s; could not retrieve processor info', platform)
  return processorStats
}
...
module.exports._getMemoryStats = async function getMemoryStats() {
  ...
  console.log('Unknown platform: %s; could not retrieve memory info', platform)
  return null
}
...
async function getKernelVersion() {
  ...
  console.log('Unknown platform: %s; could not read kernel version', platform)
  return null
}
...
async function getSysctlValue(names = []) {
  ...
  for (const name of names) {
    ...
    try {
      ...
      console.log('Error when trying to run: sysctl -n %s: %s', name, err.message)
    }
  }

  if (returnValue === null) {
    console.log('No sysctl info found for names: %j', names)
  }
