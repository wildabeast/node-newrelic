...
function NormalizerRule(json) {
  if (!json) {
    console.log('Received incompletely specified metric normalization rule from collector.')
  ...
  if (json.match_expression instanceof RegExp) {
    ...
    try {
      ...
      console.log(error, 'Problem compiling metric normalization rule pattern.')
      this.pattern = /^$/
    }
  }
}
