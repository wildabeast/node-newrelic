...
MetricNormalizer.prototype.load = function load(json) {
  if (json) {
    this.rules = []
    console.log('Received %s %s normalization rule(s) from the server', json.length, this.type)
    ...
    console.log('Loaded %s %s normalization rule(s).', this.rules.length, this.type)
  }
}
...
function processNameRule(rule, ctx) {
  if (!rule.pattern) {
    return console.error({ rule: rule }, 'Simple naming rules require a pattern.')
  }
  if (!rule.name) {
    return console.error({ rule: rule }, 'Simple naming rules require a replacement name.')
  }
...
MetricNormalizer.prototype.addSimple = function addSimple(pattern, name) {
  if (!pattern) {
    return console.error('Simple naming rules require a pattern.')
  }
