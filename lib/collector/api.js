...
CollectorAPI.prototype._retry = function _retry(ctx, error, response) {
  ...
  if (response.status === 410 || response.agentRun === AGENT_RUN_BEHAVIOR.SHUTDOWN) {
    console.error('The New Relic collector rejected this agent.')
  ...
  if (backoff.warn) {
    console.log('No connection has been established to New Relic after %d attempts.', ctx.attempts)
  }
...
CollectorAPI.prototype._runLifecycle = function _runLifecycle(method, body, callback) {
  if (!this.isConnected()) {
    console.log('Not connected to New Relic. Not calling.', method.name)
    ...
    if (response.agentRun === AGENT_RUN_BEHAVIOR.RESTART) {
      ...
      this.restart(function afterRestart(connectError) {
        if (connectError) {
          ...
          console.log('Failed to restart agent run.')
        } else {
...
function _handleResponseCode(response, endpoint, cb) {
  ...
  if (SUCCESS.has(code)) {
    ...
    console.error('Agent endpoint %s returned unexpected status %s.', endpoint, code)
    setImmediate(cb, null, CollectorResponse.discard(null))
  }
}
...
function logFailure(endpoint, code, action) {
  console.error('Agent endpoint %s returned %s status. %s.', endpoint, code, action)
}
