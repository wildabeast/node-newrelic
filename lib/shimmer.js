...
const shimmer = (module.exports = {
  ...
  wrapMethod: function wrapMethod(nodule, noduleName, methods, wrapper) {
    if (!methods) {
      console.log(new Error(), 'Must include a method name to wrap. Called from:')
      return
    }
    ...
    methods.forEach((method) => {
      ...
      if (!nodule) {
        console.log("Can't wrap %s from nonexistent object.", fqmn)
        return
      }

      if (!wrapper) {
        console.log("Can't wrap %s without a wrapper generator.", fqmn)
        return
      }
      ...
      if (original[symbols.unwrap]) {
        console.log('%s already wrapped by agent.', fqmn)
        return
      }
  ...
  wrapDeprecated: function wrapDeprecated(nodule, noduleName, property, options) {
    if (!property) {
      console.log(new Error(), 'Must include a function name to wrap. Called from:')
      return
    }
  ...
  unwrapMethod: function unwrapMethod(nodule, noduleName, method) {
    ...
    if (!nodule) {
      return console.log("Can't unwrap %s from nonexistent object.", fqmn)
    }
    ...
    if (!wrapped) {
      return console.log('%s not defined, so not unwrapping.', fqmn)
    }
    if (!wrapped[symbols.unwrap]) {
      return console.log("%s isn't unwrappable.", fqmn)
    }
    ...
    for (const [moduleName, instrInfo] of Object.entries(INSTRUMENTATIONS)) {
      if (instrInfo.module) {
        ...
        try {
          ...
          console.log('Failed to load instrumentation for ' + instrInfo.module, e)
          return
        }
      } else {
  ...
  getPackageVersion(moduleName) {
    try {
      ...
      console.log('Failed to get version for `%s`, reason: %s', moduleName, err.message)
      return process.version
    }
  }
})
...
function _firstPartyInstrumentation(agent, fileName, shim, nodule, moduleName) {
  ...
  if (!fs.existsSync(fileName)) {
    return console.log('Tried to load instrumentation from %s, but file does not exist', fullPath)
  }
  try {
...
function hasValidRegisterOptions(opts) {
  if (!opts) {
    console.log('Instrumentation registration failed, no options provided')
    return false
  }

  if (!opts.moduleName) {
    console.log(`Instrumentation registration failed, 'moduleName' not provided`)
    return false
  }
...
function trackInstrumentationUsage(agent, shim, moduleName, metricPrefix) {
  try {
    ...
    console.log('Unable to track instrumentation usage for: ', moduleName, error)
  }
}
