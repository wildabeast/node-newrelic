...
RemoteMethod.prototype.serialize = function serialize(payload, callback) {
  ...
  try {
    ...
    console.error(error, 'Unable to serialize payload for method %s.', this.name)
...
RemoteMethod.prototype._post = function _post(data, nrHeaders, callback) {
  ...
  if (options.compressed) {
    ...
    try {
      ...
      compressor(data, function onCompress(err, compressed) {
        if (err) {
          console.log(err, 'Error compressing JSON for delivery. Not sending.')
          return callback(err)
        }
  ...
  function makeRequest() {
    try {
      ...
      console.log(err, 'Failed to prepare request to collector method %s!', method.name)
      callback(err)
    }
  }
  ...
  function onResponse(response) {
    response.on('end', function onEnd() {
      console.log('Finished receiving data back from the collector for %s.', method.name)
    })
